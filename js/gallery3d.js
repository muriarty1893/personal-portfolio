...
(function() {
  ...
  // Set scroll height: enough room per project
  section.style.height = (PROJECTS.length * 80 + 40) + 'vh';
  ...
  // Floor
  var floorMat = new THREE.MeshStandardMaterial({ color: C.floor, roughness: 0.3, metalness: 0.15 });
  ...
  // End wall
  var endWall = new THREE.Mesh(new THREE.PlaneGeometry(CW, CH), wallMat.clone());
  endWall.position.set(0, CH / 2, -(CL - 4));
  scene.add(endWall);
  ...
  // Thin floor guide line
  var lineMat = new THREE.MeshBasicMaterial({ color: getIsDark() ? 0x1a1a1a : 0xaaaaaa });
  var floorLine = new THREE.Mesh(new THREE.PlaneGeometry(0.015, CL), lineMat);
  floorLine.rotation.x = -Math.PI / 2;
  floorLine.position.set(0, 0.003, -(CL / 2 - 4));
  scene.add(floorLine);

  // ============ LIGHTING ============
  var ambient = new THREE.AmbientLight(0xffffff, getIsDark() ? 0.12 : 0.4);
  scene.add(ambient);

  // Soft ceiling strip every few units
  var stripLights = [];
  for (var si = 0; si < Math.ceil(CL / 5); si++) {
    var sl = new THREE.PointLight(0xffffff, getIsDark() ? 0.08 : 0.15, 7);
    sl.position.set(0, CH - 0.15, -(si * 5) + 4);
    scene.add(sl);
    stripLights.push(sl);
  }

  // Per-painting spotlights
  var spotlights = [];
  PROJECTS.forEach(function(_, i) {
    ...
    var spot = new THREE.SpotLight(0xffffff, getIsDark() ? 2.0 : 1.5, 12, Math.PI / 5, 0.5, 1);
  ...
  // Camera z range: start just before first painting, end just after last
  var CAM_START_Z = -(OFFSET - 4);
  var CAM_END_Z   = -((PROJECTS.length - 1) * SPACING + OFFSET + 3);
  ...
  if (darkBtn) {
    darkBtn.addEventListener('click', function() {
      setTimeout(function() {
        ...
        lineMat.color.setHex(d ? 0x1a1a1a : 0xaaaaaa);
        ambient.intensity = d ? 0.12 : 0.4;
        stripLights.forEach(function(l) { l.intensity = d ? 0.08 : 0.15; });
        spotlights.forEach(function(s) { s.intensity = d ? 2.0 : 1.5; });
